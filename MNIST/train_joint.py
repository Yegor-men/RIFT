...
ema_r2ir = copy.deepcopy(r2ir)
ema_r2ir.eval()
for param in ema_r2ir.parameters():
...
optimizer = torch.optim.AdamW(
    params=list(r2ir.parameters()) + list(r2id.parameters()) + list(text_encoder.parameters()), lr=peak_lr)
...
train_losses = []
test_losses = []
recon_percentile_losses = []
eps_percentile_losses = []
...
for E in range(num_epochs):
    ...
    for i, (image, label) in tqdm(enumerate(train_dloader), total=len(train_dloader), leave=True, desc=f"E:{E}"):
        ...
        latent = r2ir.encode(image, scale=r2ir_scale)
        noisy_latent, eps = corrupt_image(latent, alpha_bar)
        reconstructed_latent = r2ir.decode(noisy_latent, scale=r2ir_scale)
        reconstruction_loss = signal_weighted_mse(reconstructed_latent, image, alpha_bar)

        pos_cond = text_encoder(label).to(device)
        null_cond = text_encoder(torch.zeros_like(label)).to(device)
        cond_list = [pos_cond, null_cond]

        predicted_eps_list = r2id(noisy_latent, alpha_bar, cond_list)
        eps_pos, eps_null = predicted_eps_list[0], predicted_eps_list[1]
        eps_loss = (nn.functional.mse_loss(eps_null, eps) + nn.functional.mse_loss(eps_pos, eps)) / 2.0

        loss = (eps_loss + reconstruction_loss) / 2.0
        ...
        scheduler.step()
        optimizer.zero_grad()
        update_ema_model(r2ir, ema_r2ir, ema_decay)
    ...
    with torch.no_grad():
        for i, (image, label) in tqdm(enumerate(test_dloader), total=len(test_dloader), leave=True, desc=f"E:{E}"):
            ...
            predicted_eps_list = r2id(noisy_latent, alpha_bar, cond_list)
            eps_pos, eps_null = predicted_eps_list[0], predicted_eps_list[1]
            eps_loss = (nn.functional.mse_loss(eps_null, eps) + nn.functional.mse_loss(eps_pos, eps)) / 2
    ...
    with torch.no_grad():
        ...
        for t in t_range:
            ...
            latent = ema_r2ir.encode(image, scale=r2ir_scale)
            noisy_latent, eps = corrupt_image(latent, alpha_bar)

            recon_img = ema_r2ir.decode(noisy_latent, scale=r2ir_scale)
            recon_loss = nn.functional.mse_loss(recon_img, image)
            ...
            predicted_eps_list = ema_r2id(noisy_latent, alpha_bar, cond_list)
            eps_pos, eps_null = predicted_eps_list[0], predicted_eps_list[1]

            pos_loss = nn.functional.mse_loss(eps_pos, eps)
            null_loss = nn.functional.mse_loss(eps_null, eps)
            eps_pos_losses.append(pos_loss.item())
            eps_null_losses.append(null_loss.item())

        x = np.linspace(0, 1, len(recon_losses))
        plt.plot(x, recon_losses, label="Reconstruction")
        percentiles = [1, 25, 50, 75, 99]
        indices = [int(p / 100 * (len(recon_losses) - 1)) for p in percentiles]
        percentile_x = [x[i] for i in indices]
        percentile_y = [recon_losses[i] for i in indices]
        for px, py, p in zip(percentile_x, percentile_y, percentiles):
            plt.scatter(px, py, color='red')
            plt.text(px, py, f'{py}', fontsize=9, ha='center', va='bottom')
        plt.title('T scrape Losses - Reconstruction')
        plt.legend()
        plt.show()

        recon_percentile_losses.append(percentile_y)
        transposed = list(zip(*recon_percentile_losses))
        for i, series in enumerate(transposed):
            plt.plot(series, label=f"t = {(percentiles[i] / 100):.2f}")
        plt.title("T scrape percentile losses over time - Reconstruction")
        plt.legend()
        plt.show()

        x = np.linspace(0, 1, len(eps_null_losses))
        plt.plot(x, eps_null_losses, label="NULL")
        plt.plot(x, eps_pos_losses, label="POS")
        percentiles = [1, 25, 50, 75, 99]
        indices = [int(p / 100 * (len(eps_null_losses) - 1)) for p in percentiles]
        percentile_x = [x[i] for i in indices]
        percentile_y = [eps_null_losses[i] for i in indices]
        for px, py, p in zip(percentile_x, percentile_y, percentiles):
            plt.scatter(px, py, color='red')
            plt.text(px, py, f'{py}', fontsize=9, ha='center', va='bottom')
        plt.title('T scrape Losses - Eps pred')
        plt.legend()
        plt.show()

        eps_percentile_losses.append(percentile_y)
        transposed = list(zip(*eps_percentile_losses))
        for i, series in enumerate(transposed):
            plt.plot(series, label=f"t = {(percentiles[i] / 100):.2f}")
        plt.title("T scrape percentile losses over time - Eps_pred")
        plt.legend()
        plt.show()
    ...
    if (E + 1) % 1 == 0 or E == num_epochs:
        r2ir_path = save_model(ema_r2ir, name=f"E{E + 1}_{test_loss:.5f}_MNIST_R2IR")
        r2id_path = save_model(ema_r2id, name=f"E{E + 1}_{test_loss:.5f}_MNIST_R2ID")
