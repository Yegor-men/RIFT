...
MODEL_PATH = None  # If None, use newest Omniglot_E*_rift.safetensors from MODEL_DIR.
...
SEED = 0
SAVE_FIGURES = False
FIGURE_DIR = Path("omniglot/media/tests")
...
def load_checkpoint(device: torch.device) -> tuple[RIFT, ClassLabelConditioner, dict]:
    model_path = Path(MODEL_PATH) if MODEL_PATH else newest_checkpoint_path(MODEL_DIR, "Omniglot")
...
def glyph_labels(count: int, num_classes: int, device: torch.device) -> torch.Tensor:
...
def run_square_resolution_trajectories(model, conditioner, model_config, device):
    labels = glyph_labels(TRAJECTORY_GLYPH_COUNT, model_config["num_classes"], device)
    ...
    for size in SQUARE_SIZES:
        ...
        show_trace_grid(
            image_trace,
            title=f"Omniglot pixel RIFT trajectory | {size}x{size}",
...
def run_aspect_ratio_grid(model, conditioner, model_config, device):
    labels = glyph_labels(FINAL_GRID_COUNT, model_config["num_classes"], device)
    ...
    for height, width in ASPECT_SIZES:
        ...
        show_tensor_grid(
            samples,
            title=f"Omniglot pixel RIFT final samples | {height}x{width}",
