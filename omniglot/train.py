...
def build_conditioner(device: torch.device, num_classes: int) -> ClassLabelConditioner:
...
def checkpoint_config(num_classes: int) -> dict:
    return {
...
def main() -> None:
    ...
    for epoch in range(EPOCHS):
        ...
        if SAMPLE_EVERY > 0 and (epoch + 1) % SAMPLE_EVERY == 0:
            render_samples(ema_model, conditioner, epoch, device, num_classes)

        if SAVE_EVERY > 0 and (epoch + 1) % SAVE_EVERY == 0:
            save_checkpoint(ema_model, conditioner, epoch, test_loss, num_classes)
